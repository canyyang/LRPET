...
def ratio_print(model,prune_index_list):
    ...
    flops, params = get_model_complexity_info(model, (3, 600, 600), as_strings=False, print_per_layer_stat=True)

    model_pruning = channel_decompose_guss(model_in=model_test, prune_index_list=prune_index_list)

    flops_svd, params_svd = get_model_complexity_info(model_pruning, (3, 600, 600), as_strings=False, print_per_layer_stat=True)
