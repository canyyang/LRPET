...
from utils.lrpet import fcConvWeightReguViaSVB, channel_decompose_guss, ratio_print
...
def init_parser():
    ...
    # Data args
    parser.add_argument('--data_path', default='/home/lab/Dataset/cifar10', type=str, help='dataset path')
    ...
    parser.add_argument('--prun_goal', default=0.0, type=float, metavar='N', help='prun gold of lrpet (default: 0)')
...
def main(args):
    ...
    ratio_print(model, default_parameters)
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if acc1 > best_acc1 and epoch >= args.epochs - 20 and epoch < args.epochs:
        ...
        model = fcConvWeightReguViaSVB(model, default_parameters)
        
        writer.add_scalar("Learning Rate", lr, epoch)

    model_test = copy.deepcopy(model)
    best_model = torch.load(os.path.join(save_path, 'best.pth'))['model_state_dict']
    model_test.load_state_dict(best_model)
    print('==> exchange to SVD')
    model_train = channel_decompose_guss(model_in=model_test, prune_index_list=default_parameters)
    acc1 = validate(val_loader, model_train, criterion, lr, args, epoch=args.epochs)
        
        
    print(Fore.RED+'*'*80)
    logger.debug(f'best top-1: {best_acc1:.2f}, final decompose top-1: {acc1:.2f}')
    print('*'*80+Style.RESET_ALL)
...
if __name__ == '__main__':
    ...
    model_name += f"-{args.tag}-{args.dataset}-PRUN[{args.prun_goal}]-Seed{args.seed}"
