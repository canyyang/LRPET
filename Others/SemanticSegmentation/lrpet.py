...
from utils.utils_fit_lrpet import fit_one_epoch
from utils.utils_lrpet import ratio_print
...
if __name__ == "__main__":
    ...
    # flops, params = get_model_complexity_info(model, (3, 473, 473), as_strings=False, print_per_layer_stat=True)
    ...
    for name, m in model.named_modules():
        if isinstance(m, nn.Conv2d) and 'backbone' in name and name != 'backbone.layer4.2.conv3' :
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            print('epoch%s: ' % epoch, time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
            #---------------------------------------#
            #   如果模型有冻结学习部分
            #   则解冻，并设置参数
            #---------------------------------------#
            ...
            fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, 
                    epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, dice_loss, focal_loss, cls_weights, aux_branch, num_classes, fp16, scaler, save_period, save_dir, local_rank, default_parameters)
