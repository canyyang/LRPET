    ...
    for name, m in model_in.named_modules():
        ...
        if isinstance(m,nn.Conv2d) and 'backbone' in name and name != 'backbone.layer4.2.conv3':
            ...
            if prun_flag == True:
                ...
                new_m = nn.Sequential(
                    OrderedDict([
                        ('C', nn.Conv2d(dim[1], r, dim[2], m.stride, m.padding, m.dilation, bias=False)),
...
def ratio_print(model,prune_index_list):
    ...
    flops_svd, params_svd = get_model_complexity_info(model_pruning, (3, 473, 473), as_strings=False, print_per_layer_stat=True)
