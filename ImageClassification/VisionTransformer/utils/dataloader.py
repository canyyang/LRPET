...
def dataload(args, augmentations, normalize, data_info):
    if args.dataset == 'CIFAR10':
        root = args.data_path + 'cifar10'
        train_dataset = datasets.CIFAR10(
            root=root, train=True, download=True, transform=augmentations)
        val_dataset = datasets.CIFAR10(
            root=root, train=False, download=False, transform=transforms.Compose([
            transforms.Resize(data_info['img_size']),
            transforms.ToTensor(),
            *normalize]))
        
    elif args.dataset == 'CIFAR100':
        root = args.data_path + 'cifar100'
